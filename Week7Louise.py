...
def main():
    ...
    print("Network Device Configuration")
    print("=" * 60)
    ...
    #Convert to  XML
    if HAS_XML_TODICT:
        xml_outpuy = to_xml(ROUTER_CONFIG)
        print("\n4, XML Format")
        print("-" * 40)
        print(xml_outpuy)
    ...
    #Invalid JSON
    invalid_json = '{"hostname": "Test" missing quotes}'
    is_valid, result = validate_json(invalid_json)
    print(f"Invalide JSONL {is_valid} -> {result}")

    #Valid YAML
    valid_yaml = "hostname: Test\ninterfaces:\n - name: eth0"
    is_valid, result = validate_yaml(valid_yaml)
    print(f"Valid YAML: {is_valid}")

    #Validation challenge
    print("\6/ challenge: Validate this config file")
    print("-" * 40)
    challenge_config = """
device:
    hostname: Router-2
...
"""
    ...
    print("Try modofying ROUTER_CONFIG and running again")
    print("=" * 60)
